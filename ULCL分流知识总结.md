...
数据从UE发出后，经gNB做GTP-U封装通过N3口到达I-UPF。I-UPF拆壳后查规则表：匹配8.8.8.8走路径A（I-UPF直连出口，做NAT），匹配Docker网段走路径C（I-UPF直连内网，不做NAT），不匹配的默认走路径B（I-UPF→N9 GTP-U→PSA-UPF→NAT→公网）。三条回包路径：谁做的NAT谁还原，回包=去程逆序。

### 2.2 三条路径区别

- **路径A**：匹配 specificPath，目标 8.8.8.8（公网），I-UPF 直出，不走 N9，做 NAT
- **路径B**：不匹配走默认，目标 google.com（公网），经 PSA-UPF 转出，走 N9，做 NAT
- **路径C**：匹配 specificPath，目标 172.17.0.1（Docker 内网），I-UPF 直出，不走 N9，不做 NAT（内网认识 10.60.0.1）

### 2.3 回包路径

- 路径A：8.8.8.8 → I-UPF（NAT还原）→ gNB → UE
- 路径B：google.com → PSA-UPF（NAT还原）→ I-UPF → gNB → UE
- 路径C：172.17.0.1 → I-UPF → gNB → UE（未做NAT，无需还原）

规律：谁做的NAT谁还原，回包=去程逆序。
...
NAT映射表记录内网IP:端口↔公网IP:端口的对应关系，去程记一笔、回程查表还原。
...
UE移动前走gNB1→I-UPF1→PSA-UPF，移动后前面的gNB和I-UPF可以换，但PSA-UPF不能换。因为UE的IP是PSA-UPF分配的，NAT映射表也在PSA-UPF上。
